...
int getcommand(Tables* data) {
  ...
  switch (functionHash(func)) {
    ...
    case HASH_show_flights:
      // Here we need to parse the arguments, we need to seperate the function
      // for this because of the switch statement
    ...
    case HASH_most_delayed_airlines_at_airport:
      // most_delayed_airlines_at_airport has only one argument so we can
      // directly use args

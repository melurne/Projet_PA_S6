    ...
    for (int j = 0; j <= data->flights.dates[i].last; j++) {
      if (strcmp(data->flights.dates[i].content[j].airline, airline_id) == 0) {
        if (notIn(seen, data->flights.dates[i].content[j].org_air)) {
          strcpy(seen.content[seen.last + 1],
                 data->flights.dates[i].content[j].org_air);
          seen.last++;
        }
        if (notIn(seen, data->flights.dates[i].content[j].dest_air)) {
          strcpy(seen.content[seen.last + 1],
                 data->flights.dates[i].content[j].dest_air);
          seen.last++;
        }
      }
    }
...
void show_flights(Tables* data,
  ...
  for (int i = 0; i < timeScale.last && count < cap; i++) {
    if (strcmp(timeScale.content[i].org_air, port_id) == 0 &&
        timeScale.content[i].sched_dep > time) {
      count++;
...
void most_delayed_airlines(Tables* data) {
  ...
  for (int i = 0; i < DAYS_IN_HASHED_YEAR; i++) {
    for (int j = 0; j <= data->flights.dates[i].last; j++) {
      if (!(data->flights.dates[i].content[j].diverted ||
            data->flights.dates[i].content[j].canceled)) {
        strcpy(averages
                   .content[averages.hash(
                       data->flights.dates[i].content[j].airline)]
                   .IATA_code,
               data->flights.dates[i].content[j].airline);
        averages
            .content[averages.hash(data->flights.dates[i].content[j].airline)]
            .sum = averages
                       .content[averages.hash(
                           data->flights.dates[i].content[j].airline)]
                       .sum +
  ...
  for (int i = 0; i < MAX_LEN_AIRLINES; i++) {
    if (averages.content[i].count != 0)
      averages.content[i].sum =
          averages.content[i].sum / averages.content[i].count;
  }
  ...
  for (int j = 0; j < 5; j++) {
    ...
    printAirline(
        accessAirline(data->airlines, averages.content[id_max].IATA_code));
...
void avg_flight_duration(Tables* data, char* port_id1, char* port_id2) {
  ...
  for (int i = 0; i < DAYS_IN_HASHED_YEAR; i++) {
    for (int j = 0; j <= data->flights.dates[i].last; j++) {
      if (!(data->flights.dates[i].content[j].diverted ||
            data->flights.dates[i].content[j].canceled) &&
          (((strcmp(data->flights.dates[i].content[j].org_air, port_id1) ==
             0) &&
            (strcmp(data->flights.dates[i].content[j].dest_air, port_id2) ==
             0)) ||
           ((strcmp(data->flights.dates[i].content[j].org_air, port_id2) ==
             0) &&
            (strcmp(data->flights.dates[i].content[j].dest_air, port_id1) ==
             0)))) {
...
void find_itinerary(Tables* data,
                    char* org_id,
                    char* dest_id,
                    int month,
                    int day,
  ...
  int count = 0;
  FlightsOnDay possibleFlights =
      data->flights.dates[data->flights.hash(month, day)];
  ...
  for (int i = 0; i <= possibleFlights.last; i++) {
    if (!possibleFlights.content[i].canceled &&
        ...
        possibleFlights.content[i].sched_arr >
            possibleFlights.content[i].sched_dep &&
        count < cap) {
  ...
  for (int i = 0; i <= possibleFlights.last; i++) {
    if (!possibleFlights.content[i].canceled &&
        !possibleFlights.content[i].diverted &&
        (strcmp(possibleFlights.content[i].org_air, org_id) == 0) &&
        possibleFlights.content[i].sched_dep > time && count < cap) {
      // printFlight(possibleFlights.content[i]);
      for (int j = 0; j < possibleFlights.last; j++) {
        if (!possibleFlights.content[j].canceled &&
            !possibleFlights.content[j].diverted &&
            (strcmp(possibleFlights.content[j].org_air,
                    possibleFlights.content[i].dest_air) == 0) &&
            (strcmp(possibleFlights.content[j].dest_air, dest_id) == 0) &&
            possibleFlights.content[j].sched_dep >
                addFormatedTime(possibleFlights.content[i].sched_arr,
                                BOARDING_DELAY) &&
            count < cap) {
          printFlight(possibleFlights.content[i]);
...
void find_multicity_itinerary(Tables* data,
                              char* origin,
  ...
  printf("%s -> %s %d-%d <%s>\n", origin, portsList.first->val,
         datesList.first->month, datesList.first->day, args);
  find_itinerary(data, origin, portsList.first->val, datesList.first->month,
                 datesList.first->day, args);
  free(args);

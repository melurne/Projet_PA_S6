...
void printAirport(Airport airport) {
  printf("%s,%s,%s,%s,%s,%f,%f\n", airport.IATA_code, airport.name,
         airport.city, airport.state, airport.country, airport.latitude,
         airport.longitude);
}
...
void printFlight(Flight f) {
  if (f.diverted || f.canceled)
    printf("%d,%d,%d,%s,%s,%s,%d,,,%d,%d,,%d,%d\n", f.month, f.day, f.weekday,
           f.airline, f.org_air, f.dest_air, f.sched_dep, f.dist, f.sched_arr,
           f.diverted ? 1 : 0, f.canceled ? 1 : 0);
  else
    printf("%d,%d,%d,%s,%s,%s,%d,%.1f,%.1f,%d,%d,%.1f,%d,%d\n", f.month, f.day,
           f.weekday, f.airline, f.org_air, f.dest_air, f.sched_dep,
           f.dep_delay, f.air_time, f.dist, f.sched_arr, f.arr_delay,
           f.diverted ? 1 : 0, f.canceled ? 1 : 0);
}

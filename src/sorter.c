...
void sortAirlines(TableAirlines* airlines) {
  ...
  { 
    if (flag != -1)
  	...
  	// We reset token to avoid all elements of content being the same due to pointers
    token = blankAirline();
  }
  fclose(fp);
  //printf("Airlines end\n");
...
void sortFlights(TableFlights* flights) {
  ...
  { 
  	if (flag != -1)
		...
		// We reset token to avoid all elements of the tree being the same due to pointers
		token = blankFlight();
  }
  fclose(fp);
  //printf("Airports end\n");
